...
public class TariffController : ControllerBase
{
    ...
    [HttpGet("resourceType/{resourceType}")]
    public async Task<ActionResult<IEnumerable<TariffViewModel>>> GetByResourceType(ResourceType resourceType)
    {
        try
        {
            var tariffs = await _tariffService.GetByResourceTypeAsync(resourceType);
            return Ok(tariffs);
        }
        catch (Exception ex)
        {
