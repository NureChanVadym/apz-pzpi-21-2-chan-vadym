...
namespace EcoMeChan.Repositories
{
    public class ConsumptionRepository : IConsumptionRepository
    {
        ...
        public async Task<IEnumerable<Consumption>> GetByUserIdAsync(int userId, ResourceType resourceType, DateTime startDate, DateTime endDate)
        {
            return await _context.Consumptions
                .Where(c => c.UserId == userId && c.ResourceType == resourceType && c.Date >= startDate && c.Date <= endDate)
                .ToListAsync();
        }
